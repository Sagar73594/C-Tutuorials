...
public:
    // individulaly maiking frined

    int sumrealcomplex(Complex, Complex); // here only define the datatype complex dont write o1 or o2 other wise compiler will get cinfused and throw error.
    int sumIMcomplex(Complex, Complex);   // only declare
};

class Complex
{
    // here we have make the friend both the class by friend using and scope resolutor to make the friend of calc to complex/
...
//* KEY
// if u want to make the class A friend of  your Class B then just write friend class A;
// if u want to make one function of that class as friend then write   // friend int calc ::sumrealcomplex(Complex, Complex);